...
if !get(g:, 'python_pep8_indent_skip_concealed', 0) || !has('conceal')
        ...
        fun! s:_skip_special_chars(line, col)
...
fun! s:find_opening_paren(lnum, col)
...
fun! s:find_start_of_multiline_statement(lnum)
        let lnum = a:lnum
        while lnum > 0
                if getline(lnum - 1) =~# '\\$'
...
fun! s:find_start_of_block(lnum, types, skip, multiple) abort
        let r = []
...
fun! s:match_expr_on_line(expr, lnum, start, ...)
...
fun! s:indent_like_opening_paren(lnum)
...
fun! s:indent_like_previous_line(lnum)
